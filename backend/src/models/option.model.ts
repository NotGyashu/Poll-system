...
export class OptionModel {
  ...
  static async findById(id: string): Promise<Option | null> {
    const result = await query<Option>('SELECT * FROM options WHERE id = $1', [id]);
  ...
  static async create(pollId: string, text: string, displayOrder: number): Promise<Option> {
    const result = await query<Option>(
      `INSERT INTO options (poll_id, text, display_order)
       VALUES ($1, $2, $3)
       RETURNING *`,
      [pollId, text, displayOrder]
    );
