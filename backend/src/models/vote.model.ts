...
export class VoteModel {
  ...
  static async getVoteCounts(pollId: string): Promise<VoteCount[]> {
    const result = await query<{ option_id: string; option_text: string; count: string }>(
      `SELECT 
        o.id as option_id,
    ...
    const totalVotes = result.rows.reduce((sum, row) => sum + parseInt(row.count, 10), 0);

    return result.rows.map((row) => ({
      option_id: row.option_id,
      option_text: row.option_text,
      count: parseInt(row.count, 10),
      percentage: totalVotes > 0 ? Math.round((parseInt(row.count, 10) / totalVotes) * 100) : 0,
    }));
  }

  
  static async getPollResults(pollId: string): Promise<PollResults | null> {
    const pollResult = await query<{ id: string; question: string }>(
